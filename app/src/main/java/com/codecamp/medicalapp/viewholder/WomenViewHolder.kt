...
class WomenViewHolder(itemView : View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(women: Women){
        mUserImage.setImageResource(women.image)
        mUserDesignation.text = women.designation
    }

    fun onClick(widget : View, context: Context, onDoSomething : (position : Int, holder : WomenViewHolder) -> Unit, position: Int, holder: WomenViewHolder){
    ...
    interface onItemClick{
        fun onNavigateToDetailsPage(position: Int, holder: WomenViewHolder)
    }
}